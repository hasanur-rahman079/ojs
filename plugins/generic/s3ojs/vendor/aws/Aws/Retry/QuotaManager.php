...
class QuotaManager
{
    ...
    {
        $this->initialRetryTokens = $config['initial_retry_tokens']
            ?? 500;
        $this->noRetryIncrement = $config['no_retry_increment']
            ?? 1;
        $this->retryCost = $config['retry_cost']
            ?? 5;
        $this->timeoutRetryCost = $config['timeout_retry_cost']
            ?? 10;
